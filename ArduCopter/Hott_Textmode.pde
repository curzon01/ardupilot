...
char const _LOW_VOLT[] PROGMEM         = "FS_BATT_VOLTAGE";
char const _BATT_CAPACITY[] PROGMEM    = "FS_BATT_MAH";
...
void  HOTT_PrintWord_P(uint8_t pos,   const prog_char_t *w, bool inverted) {
  for (uint8_t index = 0; ; index++) {
...
char *  HOTT_GetParamAsChar( const char *key) {
	...
	switch (var_type) {
		...
		case AP_PARAM_INT32:
		{
			...
			sprintf(_hott_str_convert_buffer,"%ld",t);
			break;
		}
		case AP_PARAM_FLOAT:
		{
...
void  HOTT_Set_Param(const char *key, float value) {
    ...
    switch (var_type) {
    case AP_PARAM_INT8:
      value = constrain_int16(value, -128, 127);
      if (strncmp (key, "TUNE", 16) == 0 ) value = constrain_int16(value, 0, 30);
      ((AP_Int8 *)vp)->set_and_save(value);
      break;
    case AP_PARAM_INT16:
      value = constrain_int16(value, -32768, 32767);
      ((AP_Int16 *)vp)->set_and_save(value);
      break;
    case AP_PARAM_INT32:
      value = constrain_int32(value, 0xffffffff, 0x7fffffff);
	...
	for (uint8_t index = 0; index <  HOTT_PARAMS_PER_PAGE; index++) {
		...
		if (p_index <  num_parameters ) {
			...
			if((index) == selectedRow) {
				sign = '*';
				if(pgm_read_byte(&settings[p_index].editable)) {
					sign = '>';
				}
			}
			else {
